...
exports.registerClient = async (request, response) => {
  if (request.session.loggedin && request.session.username === 'admin') {
    ...
    query.on('result', () => {
      ...
      const createPaymentSQL = `CREATE TABLE IF NOT EXISTS payment_${fssaiCode} ( orderID int(11) not NULL, amount int(11) not NULL, isbilled tinyint(1) DEFAULT '0', orderdate varchar(12), tableno int not null)`;
